...
cv2.createTrackbar('max_rpm', THR_STR_WINDOW_NAME, zero_rpm, max_rpm, callback)
...
class Calibration(Node):
    ...
    def live_calibration_values(self, data):
        ...
        # max_rpm_param = rclpy.parameter.Parameter('max_rpm', rclpy.Parameter.Type.INTEGER, self.max_rpm)
        # self.set_parameters([max_rpm_param])
        ...
        self.steering_value.data = Steering_sensitivity*slider_to_normalized(steer_input)
        self.throttle_value.data = slider_to_normalized(throttle_input)

        self.steering_publisher.publish(self.steering_value)
        self.throttle_publisher.publish(self.throttle_value)
...
def main(args=None):
    ...
    try:
        ...
        # Stop the car
        CAL_publisher.throttle_value.data = CAL_publisher.zero_throttle
        CAL_publisher.throttle_publisher.publish(CAL_publisher.throttle_value)
